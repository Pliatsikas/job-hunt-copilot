...
**Α8 — Ο default provider στο deployment είναι ο Groq, με μετρημένα (όχι δημοσιευμένα) όρια.**
Το free tier του Gemini κόβει **ανά μοντέλο ανά μέρα** — μετρημένο στα 20 requests/μέρα για το
`gemini-3.5-flash`. Με per-user όριο 50 calls, ένας χρήστης είχε δικαίωμα σε 2.5 φορές τη
συνολική ημερήσια χωρητικότητα του project: το όριο δεν μπορούσε να χτυπήσει ποτέ πριν τον
τοίχο του provider, δηλαδή δεν προστάτευε τίποτα. Το `openai/gpt-oss-120b` του Groq απαντά με
`x-ratelimit-limit-requests: 1000` (reset 86.4s = 86400/1000 → ημερήσιο bucket) και
`x-ratelimit-limit-tokens: 8000` (reset <1s → ανά λεπτό), δηλαδή 50× η ημερήσια χωρητικότητα.
Ο Gemini μένει πλήρως configured ως provider σύγκρισης για τα evals του M9 και ως fallback.
Κάθε όριο έρχεται από env με defaults στον κώδικα, ώστε η αλλαγή provider να είναι αλλαγή
τιμών και όχι αλλαγή λογικής· το προφίλ του Gemini είναι γραμμένο σχολιασμένο στο `.env.example`.

**Α9 — Μετράμε και requests και tokens, και επιβάλλεται όποιο χτυπήσει πρώτο.**
Το §6.1 ζητούσε tokens, με σωστό σκεπτικό για το κόστος. Αλλά το free tier μετράει *requests*:
μια κλήση 200 tokens και μια 8.000 στοιχίζουν το ίδιο 1/1000 της ημέρας. Οπότε το
`UsageCounter` κρατάει `calls`, `inputTokens` και `outputTokens`, και ο έλεγχος περνάει και
από τα δύο. Μετρημένο κόστος ανάλυσης: 2.033–2.979 tokens (μέσος όρος 2.452, n=4).

**Α10 — Το IP rate limiting ζει στο `authorize()`, όχι στο sign-in server action.**
Το Auth.js δέχεται και απευθείας POST στο `/api/auth/callback/credentials`. Ένας limiter μόνο
στο server action θα κάλυπτε μόνο τη διαδρομή που ακολουθεί μια φόρμα browser — όχι αυτή που
θα ακολουθούσε ένα script, που είναι ακριβώς η απειλή. Το `authorize()` είναι το ένα σημείο
απ' όπου περνάνε και οι δύο. Το registration υπάρχει μόνο ως server action, οπότε εκεί ο
έλεγχος μένει στο action.

**Α11 — Το LLM budget αποτυγχάνει κλειστά, ο auth limiter ανοιχτά.**
Αν ο έλεγχος budget δεν μπορεί να τρέξει, η κλήση δεν γίνεται: το να ξοδεύεις πραγματικά
χρήματα πάνω σε budget που δεν μπορείς να επαληθεύσεις είναι η χειρότερη από τις δύο
αποτυχίες. Αν ο counter του auth limiter δεν είναι προσβάσιμος, η προσπάθεια επιτρέπεται:
ένας χαλασμένος counter εκεί θα κλείδωνε κάθε πραγματικό χρήστη έξω από ένα κατά τα άλλα υγιές
app, και αυτό που περιορίζεται είναι φθηνό. Και τα δύο λογαριάζονται στο log, ποτέ σιωπηλά.

**Α12 — Τα contact details αφαιρούνται στον κώδικα, πριν τον provider, και η οθόνη review είναι
υποχρεωτική.** Email και τηλέφωνο κόβονται ντετερμινιστικά (`lib/cv-import/redact.ts`) πριν το
κείμενο φύγει από τον server· το cleanup prompt είναι το δεύτερο δίχτυ για ό,τι δεν πιάνει ένα
pattern (διεύθυνση, ημερομηνία γέννησης), και η οθόνη review το τρίτο. Τα URLs μένουν: ένα link
GitHub είναι απόδειξη, όχι κανάλι επικοινωνίας, και ο χρήστης το σβήνει με ένα πάτημα. Η
εξαγωγή γίνεται με `unpdf` (μηδέν dependencies, φτιαγμένο για serverless) και το cleanup pass
ζητάει `{ lines: string[] }` μέσα από το ίδιο `completeWithRepair` με την ανάλυση — η μορφή
«μία πρόταση ανά γραμμή» είναι schema, όχι ελπίδα. Το import μετράει στο budget του χρήστη
όπως κάθε άλλη κλήση. Επιτυχία μετρημένη όπως ορίζει το §6.2: δίστηλο PDF → import → ανάλυση με
5 matched skills και 0 dropped claims.

**Α13 — Το tailored CV είναι επιλογή και αναδιάταξη αυτούσιων γραμμών, όχι επαναδιατύπωση.**
Το §6.3 επέτρεπε «αναδιάταξη και επαναδιατύπωση»· η επαναδιατύπωση κόπηκε μετά από ρητή
απαίτηση του owner (15/9): το μοντέλο δεν ξαναγράφει τα λόγια του. Κάθε γραμμή του
αποτελέσματος πρέπει να **ταυτίζεται** με ολόκληρη γραμμή του `cvText` μετά την ίδια
κανονικοποίηση με το grounding της ανάλυσης (`lib/llm/tailor-grounding.ts`) — όχι απλώς να
είναι substring, γιατί μισή πρόταση είναι επεξεργασία. Ό,τι διαφέρει σε έναν χαρακτήρα
πετιέται και **εμφανίζεται** στον χρήστη ως «left out», δεν κρύβεται. Αν πάνω από το ένα τρίτο
των γραμμών απορριφθεί, δεν αποθηκεύεται τίποτα. Αποθηκεύεται το κείμενο του CV, όχι το
αντίγραφο του μοντέλου. Ο fabrication detector του M5 τρέχει και εδώ όπως ζητά το §6.3, αλλά
με exact-line grounding δεν μπορεί να βρει τίποτα εκ κατασκευής — το «0» του είναι επιβεβαίωση,
