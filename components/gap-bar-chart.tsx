...
const SEVERITY_STYLE: Record<Severity, { bar: string }> = {
  blocker: { bar: "bg-destructive" },
  important: { bar: "bg-primary" },
  nice_to_have: { bar: "bg-muted-foreground/40" },
};
 ...
 */
export function GapBarChart({ skills, t }: { skills: MissingSkill[]; t: T }) {
      ...
      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
        {(Object.keys(SEVERITY_STYLE) as Severity[]).map((severity) => (
          <span key={severity} className="flex items-center gap-1.5">
            ...
            {t(`application.severity.${severity}`)}
          </span>
        ))}
      </div>
